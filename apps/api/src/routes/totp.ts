...
export function createTotpRoutes(
  dependencies: TotpRouteDependencies,
) {
  return async function totpRoutes(
    app: FastifyInstance,
  ): Promise<void> {
    app.post(
      "/auth/totp/setup",
      async (request) => {
        let body: z.infer<
          typeof createFactorSchema
        >;

        try {
          body =
            createFactorSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
        ...
        try {
          const result =
            await dependencies.totpService.createFactor(
    ...
    app.post(
      ...
      async (request) => {
        let body: z.infer<
          typeof enableFactorSchema
        >;

        try {
          body =
            enableFactorSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
            400,
            "INVALID_REQUEST",
            "Invalid TOTP enable request",
          );
        }

        try {
          const factor =
            await dependencies.totpService.enableFactor(
              body.factorId,
              body.code,
            );
    ...
    app.post(
      ...
      async (request) => {
        let body: z.infer<
          typeof disableFactorSchema
        >;

        try {
          body =
            disableFactorSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
            400,
            "INVALID_REQUEST",
            "Invalid TOTP disable request",
          );
        }

        try {
          const factor =
            await dependencies.totpService.disableFactor(
              body.factorId,
            );
    ...
    app.post(
      ...
      async (request) => {
        ...
        try {
          const valid =
            await dependencies.totpService.verifyCode({
              identityAccountId:
                body.identityAccountId,
              code: body.code,
            });
  ...
  switch (error.message) {
    case "TOTP factor not found":
      return new ApiError(
        ...
        "TOTP factor not found",
      );

    case "TOTP factor is disabled":
      return new ApiError(
        409,
        "TOTP_FACTOR_DISABLED",
        "TOTP factor is disabled",
      );
