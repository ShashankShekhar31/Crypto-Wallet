...
const registrationOptionsSchema =
  z.object({
    identityAccountId: z.string().uuid(),
  });

const authenticationOptionsSchema =
  z.object({
    identityAccountId: z.string().uuid(),
  });
...
const registrationResponseSchema =
  z.object({
    ...
    type: z.string().min(1),
    clientExtensionResults: z.record(
      z.string(),
      z.unknown(),
    ),
...
const finishRegistrationSchema =
  ceremonySchema.extend({
    response: registrationResponseSchema,
  });

const finishAuthenticationSchema =
  ceremonySchema.extend({
...
export function createPasskeyRoutes(
  ...
  return async function passkeyRoutes(
    app: FastifyInstance,
  ): Promise<void> {
    app.post(
      ...
      async (request) => {
        let body: z.infer<
          typeof registrationOptionsSchema
        >;

        try {
          body =
            registrationOptionsSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
        ...
        try {
          const result =
            await dependencies.passkeyService
              .startRegistration({
                identityAccountId:
    ...
    app.post(
      ...
      async (request) => {
        ...
        try {
          body =
            finishRegistrationSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
    ...
    app.post(
      ...
      async (request) => {
        ...
        try {
          body =
            authenticationOptionsSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
    ...
    app.post(
      ...
      async (request) => {
        let body: z.infer<
          typeof finishAuthenticationSchema
        >;

        try {
          body =
            finishAuthenticationSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
