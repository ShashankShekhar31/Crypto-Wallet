...
export function createRecoveryRoutes(
  ...
  return async function recoveryRoutes(
    app: FastifyInstance,
  ): Promise<void> {
    app.post(
      "/auth/recovery/setup",
      async (request) => {
        let body: z.infer<
          typeof identityAccountSchema
        >;

        try {
          body =
            identityAccountSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
    ...
    app.post(
      ...
      async (request) => {
        let body: z.infer<
          typeof consumeCodeSchema
        >;

        try {
          body =
            consumeCodeSchema.parse(
              request.body,
            );
        } catch {
          throw new ApiError(
