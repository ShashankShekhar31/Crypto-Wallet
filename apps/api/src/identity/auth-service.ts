...
import { IdentityRepository, type IdentityAccountRecord } from "./repository.js";
...
export class AuthenticationService {
  ...
  async authenticateWithPassword(
    ...
    const identityAccount = await this.identityRepository.findByEmail(input.normalizedEmail);
    ...
    const credential = await this.identityRepository.findPasswordCredential(identityAccount.id);
    ...
    if (credential.lockedUntil !== null && credential.lockedUntil > new Date()) {
      await this.authEventRepository.record({
        userId: identityAccount.userId,
        eventType: "password_login",
        outcome: "blocked",
    ...
    if (!valid) {
      await this.identityRepository.recordPasswordFailure(identityAccount.id, {
        maxAttempts: PASSWORD_MAX_ATTEMPTS,
        lockDurationMs: PASSWORD_LOCK_DURATION_MS,
      });
    ...
    const device = await this.deviceRepository.findActiveDeviceForUser(
      input.deviceId,
    ...
    await this.identityRepository.resetPasswordFailures(identityAccount.id);

    const loginRisk = await this.loginRiskService.evaluate({
      userId: identityAccount.userId,
    ...
    if (loginRisk.suspicious) {
      await this.authEventRepository.record({
        ...
        failureCode: loginRisk.reasons.join(","),
      });
    }
