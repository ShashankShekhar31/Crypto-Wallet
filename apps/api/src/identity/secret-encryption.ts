...
export class SecretEncryption {
  constructor(
    ...
    if (keyVersion.trim().length === 0) {
      throw new Error("Secret encryption key version is required");
    }
  }
  ...
  decrypt(encrypted: EncryptedSecret): string {
    if (encrypted.keyVersion !== this.keyVersion) {
      throw new Error("Unsupported secret encryption key version");
    }

    if (encrypted.nonce.length !== NONCE_BYTES) {
      throw new Error("Invalid secret encryption nonce");
    }
    ...
    const authTagOffset = encrypted.ciphertext.length - AUTH_TAG_BYTES;

    const ciphertext = encrypted.ciphertext.subarray(0, authTagOffset);
