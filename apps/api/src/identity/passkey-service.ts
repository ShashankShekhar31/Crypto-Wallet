...
const REGISTRATION_CHALLENGE_TYPE = "registration" as const;

const AUTHENTICATION_CHALLENGE_TYPE = "authentication" as const;
...
export interface FinishedPasskeyAuthentication {
  identityAccountId: string;
  passkey: Awaited<ReturnType<PasskeyRepository["findByCredentialId"]>>;
}
...
export class PasskeyService {
  ...
  constructor(options: PasskeyServiceOptions) {
    ...
    this.passkeyRepository = options.passkeyRepository;
  ...
  async startRegistration(
    ...
    validateIdentityAccountId(input.identityAccountId);

    const identity = await this.identityRepository.findById(input.identityAccountId);
    ...
    const existingPasskeys = await this.passkeyRepository.findActiveByIdentityAccountId(
      identity.id,
    );
    ...
    await this.challengeStore.save(ceremonyId, {
  ...
  async finishRegistration(input: FinishPasskeyRegistrationInput) {
    if (typeof input.ceremonyId !== "string" || input.ceremonyId.trim().length === 0) {
      throw new Error("Passkey ceremony ID is required");
    }

    const challenge = await this.challengeStore.consume(input.ceremonyId);
    ...
    if (challenge.type !== REGISTRATION_CHALLENGE_TYPE) {
      throw new Error("Invalid passkey ceremony type");
    }

    if (!challenge.identityAccountId) {
      throw new Error("Passkey identity account is missing");
    }

    const identity = await this.identityRepository.findById(challenge.identityAccountId);
    ...
    if (!registrationInfo) {
      throw new Error("Passkey registration information is missing");
    }

    const credential = registrationInfo.credential;

    const existing = await this.passkeyRepository.findByCredentialId(
    ...
    const passkey = await this.passkeyRepository.createCredential({
      identityAccountId: identity.id,
  ...
  async startAuthentication(
    ...
    const passkeys = await this.passkeyRepository.findActiveByIdentityAccountId(identity.id);
  ...
  async finishAuthentication(
    ...
    const passkey = await this.passkeyRepository.findByCredentialId(credentialId);
    ...
    if (passkey.revokedAt !== null) {
      throw new Error("Passkey credential is revoked");
    }

    if (passkey.identityAccountId !== identity.id) {
      throw new Error("Passkey credential does not belong to identity account");
    }

    const publicKey = new Uint8Array(passkey.publicKey.length);
    ...
    if (newCounter < passkey.signCount) {
      throw new Error("Passkey signature counter rollback detected");
    }

    if (newCounter > passkey.signCount) {
      const updated = await this.passkeyRepository.updateSignCount(passkey.id, newCounter);

      if (!updated) {
        throw new Error("Failed to update passkey signature counter");
      }
    }

    const used = await this.passkeyRepository.markUsed(passkey.id);

    if (!used) {
      throw new Error("Failed to mark passkey as used");
    }
...
function uuidToBytes(value: string): Uint8Array<ArrayBuffer> {
  ...
  if (!/^[0-9a-fA-F]{32}$/.test(normalized)) {
    throw new Error("Identity account ID must be a UUID");
  }
