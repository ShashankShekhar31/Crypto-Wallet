...
if (!databaseUrl) {
  throw new Error("DATABASE_URL is required for session service tests");
}
...
describe("SessionService", () => {
    ...
    try {
      ...
      await storage.query(
        `
          ...
          VALUES ($1, $2, $3, $4)
        `,
        [deviceId, userId, "test", "session-service-test"],
      );
      ...
      expect(result.previousSession).toMatchObject({
        ...
        replacedBySessionId: result.session.id,
      });
      ...
      expect(result.session.expiresAt.getTime()).toBe(expiresAt.getTime());
  ...
  it("rejects an unknown refresh token", async () => {
    ...
    const service = new SessionService(new SessionRepository(storage));
  ...
  it("rejects a rotated refresh token", async () => {
    ...
    try {
      ...
      expect(replacement).toMatchObject({
        ...
        status: "revoked",
        tokenFamilyId: refreshed.session.tokenFamilyId,
  ...
  it("rejects an expired session", async () => {
    ...
    try {
      ...
      ).rejects.toThrow("Auth session has expired");
    } finally {
      await storage.query(
        `
        DELETE FROM devices
  ...
  it("rejects an idle-expired session", async () => {
    ...
    try {
      ...
      ).rejects.toThrow("Auth session is idle-expired");
    } finally {
      await storage.query(
        `
        DELETE FROM devices
