...
describe("TotpService", () => {
  it("creates a factor with an encrypted secret", async () => {
    ...
    const encrypted =
      encryption.encrypt("placeholder");
    ...
    expect(
      ...
      Buffer.from(result.secret),
    );

    expect(input.secretNonce.length)
      .toBeGreaterThan(0);
  ...
  it("verifies a valid TOTP code", async () => {
    ...
    vi.mocked(
      repository.findActiveByIdentityAccountId,
    ).mockResolvedValue(factor);
