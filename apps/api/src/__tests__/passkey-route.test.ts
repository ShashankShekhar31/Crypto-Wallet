...
async function createApp(passkeyService: PasskeyService): Promise<FastifyInstance> {
  const app = Fastify({
...
describe("Passkey routes", () => {
  it("starts passkey registration successfully", async () => {
    const passkeyService = createPasskeyServiceMock();

    vi.mocked(passkeyService.startRegistration).mockResolvedValue({
    ...
    const app = await createApp(passkeyService);

    const response = await app.inject({
      method: "POST",
      url: "/auth/passkey/register/options",
      payload: {
        identityAccountId: "00000000-0000-4000-8000-000000000002",
      },
    });
    ...
    expect(passkeyService.startRegistration).toHaveBeenCalledWith({
      identityAccountId: "00000000-0000-4000-8000-000000000002",
    });
  });
  ...
  it("rejects invalid registration options requests", async () => {
    ...
    expect(response.json()).toEqual({
      error: {
        ...
        message: "Invalid passkey registration options request",
      },
    });

    expect(passkeyService.startRegistration).not.toHaveBeenCalled();
  });
  ...
  it("finishes passkey registration successfully", async () => {
    ...
    vi.mocked(passkeyService.finishRegistration).mockResolvedValue({
      identityAccount: {
        id: "identity-id",
        userId: "user-id",
    ...
    expect(passkeyService.finishRegistration).toHaveBeenCalledOnce();
  });

  it("starts passkey authentication successfully", async () => {
    const passkeyService = createPasskeyServiceMock();

    vi.mocked(passkeyService.startAuthentication).mockResolvedValue({
      ceremonyId: "00000000-0000-4000-8000-000000000003",
      options: {
    ...
    expect(passkeyService.startAuthentication).toHaveBeenCalledWith({
      identityAccountId: "00000000-0000-4000-8000-000000000002",
    });
  });
  ...
  it("rejects invalid authentication options requests", async () => {
    ...
    expect(response.json()).toEqual({
      error: {
        code: "INVALID_REQUEST",
        message: "Invalid passkey authentication options request",
      },
    });

    expect(passkeyService.startAuthentication).not.toHaveBeenCalled();
  });

  it("finishes passkey authentication successfully", async () => {
    const lastUsedAt = new Date("2026-08-27T12:05:00.000Z");

    const passkeyService = createPasskeyServiceMock();

    vi.mocked(passkeyService.finishAuthentication).mockResolvedValue({
      identityAccountId: "identity-id",
      passkey: {
        id: "passkey-id",
        identityAccountId: "identity-id",
    ...
    expect(response.json()).toEqual({
      data: {
        ...
        passkeyId: "passkey-id",
        authenticated: true,
        lastUsedAt: "2026-08-27T12:05:00.000Z",
      },
      requestId: "test-request-id",
    });

    expect(passkeyService.finishAuthentication).toHaveBeenCalledWith({
      ceremonyId: "00000000-0000-4000-8000-000000000003",
      response: validAuthenticationResponse(),
    });
  });
  ...
  it("maps passkey credential errors correctly", async () => {
    ...
    vi.mocked(passkeyService.finishAuthentication).mockRejectedValue(
      new Error("Passkey credential not found"),
    );
  ...
  it("maps identity mismatch errors correctly", async () => {
    ...
    vi.mocked(passkeyService.finishAuthentication).mockRejectedValue(
      new Error("Passkey credential does not belong to identity account"),
    );
    ...
    expect(response.json()).toEqual({
      error: {
        ...
        message: "Passkey credential does not belong to identity account",
      },
    });
  });
  ...
  it("maps invalid passkey ceremony errors correctly", async () => {
    ...
    vi.mocked(passkeyService.finishAuthentication).mockRejectedValue(
      new Error("Passkey challenge not found or expired"),
    );
  ...
  it("rejects malformed authentication verification requests", async () => {
    ...
    expect(response.json()).toEqual({
      error: {
        code: "INVALID_REQUEST",
        message: "Invalid passkey authentication verification request",
      },
    });

    expect(passkeyService.finishAuthentication).not.toHaveBeenCalled();
  });
});
