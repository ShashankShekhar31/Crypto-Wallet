...
export class DefaultErc20BalanceReader
  implements Erc20BalanceReader
{
  ...
  async getBalance(
    ...
    const normalizedOwnerAddress =
      validateEvmAddress(ownerAddress);
...
export function encodeErc20BalanceOf(
  ...
  return `0x${ERC20_BALANCE_OF_SELECTOR}${addressWithoutPrefix.padStart(
  64,
  "0",
)}`;
}

export function decodeErc20Uint256(
  value: string,
): bigint {
