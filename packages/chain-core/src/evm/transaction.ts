...
function validateNonNegativeQuantity(name: string, value: bigint): void {
  if (value < 0n) {
    throw new Error(`${name} must be non-negative`);
  }
}

function validateHexData(value: string): string {
...
export function createEvmUnsignedTransaction(
  ...
  validateNonNegativeQuantity("EVM transaction gas limit", request.gasLimit);

  validateNonNegativeQuantity("EVM transaction max fee per gas", request.maxFeePerGas);
  ...
  if (request.maxPriorityFeePerGas > request.maxFeePerGas) {
    throw new Error("EVM transaction max priority fee per gas cannot exceed max fee per gas");
  }

  const value = request.value ?? 0n;

  validateNonNegativeQuantity("EVM transaction value", value);

  const to = validateEvmAddress(request.to);

  const data = validateHexData(request.data ?? "0x");
