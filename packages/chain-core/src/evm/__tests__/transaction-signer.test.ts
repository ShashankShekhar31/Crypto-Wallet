...
describe("DefaultEvmTransactionSigner", () => {
  ...
  it("signs the EIP-1559 transaction digest", async () => {
    ...
    const walletSigner: WalletSigner = {
      signDigest(_key, digest) {
        receivedDigest = new Uint8Array(digest);
    ...
    const signer = new DefaultEvmTransactionSigner(walletSigner);

    const signature = await signer.signTransaction(transaction, key);

    expect(signature.compact).toHaveLength(64);

    expect(signature.recovery).toBe(1);

    expect(receivedDigest).toEqual(createEip1559SigningDigest(transaction));
  });
  ...
  it("passes the same derived key to the wallet signer", async () => {
    ...
    await signer.signTransaction(transaction, key);

    expect(receivedKey).toBe(key);
  });

  it("returns a defensive copy of the signature", async () => {
    const compact = new Uint8Array(64).fill(0x22);
    ...
    expect(signature.compact[0]).toBe(0x22);
  });
  ...
  it("preserves the recovery identifier", async () => {
    ...
    expect(signature.recovery).toBe(3);
  });
