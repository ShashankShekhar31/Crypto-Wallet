...
describe("EIP-1559 transaction signing", () => {
  ...
  it("rejects signatures that are not 64 bytes", () => {
    ...
    expect(() =>
      ...
      "EVM transaction signature must be exactly 64 bytes",
    );
  });
  ...
  it("rejects an invalid recovery identifier", () => {
    ...
    expect(() =>
      ...
      "EVM transaction recovery identifier must be between 0 and 3",
    );
  });
  ...
  it("uses recovery parity for yParity", () => {
    ...
    expect(
      Buffer.from(recoveryZero).toString("hex"),
    ).not.toBe(
  ...
  const digest =
    createEip1559SigningDigest(transaction);

  expect(digest).toHaveLength(32);

  expect(digest).toEqual(
    hashEip1559SigningPayload(transaction),
  );
});
});