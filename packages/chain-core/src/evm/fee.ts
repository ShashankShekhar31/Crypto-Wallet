...
const ETH_MAX_PRIORITY_FEE_METHOD =
...
export async function estimateEvmTransactionFees(
  ...
  const gasLimit = await provider.request<string>(
    ETH_ESTIMATE_GAS_METHOD,
    [
      {
        to: transaction.to,
        value: toRpcQuantity(
  ...
  const gasPrice = await provider.request<string>(
    ETH_GAS_PRICE_METHOD,
    [],
  );

  const maxPriorityFeePerGas =
    await provider.request<string>(
      ETH_MAX_PRIORITY_FEE_METHOD,
      [],
    );

  const parsedGasLimit =
    parseEvmQuantity(gasLimit);

  const parsedGasPrice =
    parseEvmQuantity(gasPrice);

  const parsedMaxPriorityFeePerGas =
    parseEvmQuantity(maxPriorityFeePerGas);
