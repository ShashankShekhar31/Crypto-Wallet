...
import type { SolanaInstruction, SolanaUnsignedTransaction } from "./transaction.js";
...
function normalizeAccountMeta(account: SolanaAccountMeta): SolanaAccountMeta {
  const address = validateSolanaAddress(account.address);
...
function addAccount(
  ...
  const existingIndex = indexes.get(normalized.address);

  if (existingIndex !== undefined) {
    const existing = accounts[existingIndex];

    if (existing === undefined) {
      throw new Error("Solana account index is inconsistent");
    }

    accounts[existingIndex] = mergeAccountMeta(existing, normalized);
  ...
  indexes.set(normalized.address, index);
...
function compileInstruction(
  ...
  const programIdIndex = addAccount(accounts, indexes, {
    address: programId,
    isSigner: false,
    isWritable: false,
  });
...
export function compileSolanaTransactionMessage(
  ...
  const feePayer = validateSolanaAddress(transaction.feePayer);

  const recentBlockhash = transaction.recentBlockhash.trim();
  ...
  const instructions = transaction.instructions.map((instruction) =>
    compileInstruction(instruction, accounts, indexes),
  );

  return Object.freeze({
    feePayer,
    recentBlockhash,
    accountKeys: Object.freeze([...accounts]),
