...
export class ExchangeAssetAccountRegistry {
  private readonly assetAccounts = new Map<
    string,
  ...
  create(
    assetAccount: ExchangeAssetAccountRecord,
  ): ExchangeAssetAccountRecord {
    if (this.assetAccounts.has(assetAccount.id)) {
      throw new Error(
        `Exchange asset account already exists: ${assetAccount.id}`,
      );
    }
    ...
    if (this.identities.has(identity)) {
      throw new Error(
        "Exchange asset account already exists for exchange account and asset",
      );
    }
  ...
  private getIdentity(
    ...
    return JSON.stringify([
      assetAccount.exchangeAccountId,
  ...
  if (assetAccount.exchangeAccountId.trim().length === 0) {
    throw new Error(
      "Exchange asset account exchange account ID must not be empty",
    );
  }

  if (assetAccount.assetId.trim().length === 0) {
    throw new Error(
      "Exchange asset account asset ID must not be empty",
    );
  }

  if (assetAccount.networkId.trim().length === 0) {
    throw new Error(
      "Exchange asset account network ID must not be empty",
    );
  }
}
}