...
describe("MemorySecureStorageAdapter", () => {
  it("starts locked", () => {
    const vault = new WalletVault(new MemoryAdapter(), new TestCipher(), {
  ...
  it("creates a vault with the production Web Crypto cipher", async () => {
    ...
    vault.set("wallet-secret", new Uint8Array([10, 20, 30]));
  ...
  it("stores, returns, and removes values in memory", async () => {
    ...
    expect(await adapter.get("wallet-secret")).toEqual(new Uint8Array([1, 2, 3]));
  ...
  it("returns defensive copies from memory storage", async () => {
    ...
    await adapter.set("wallet-secret", new Uint8Array([1, 2, 3]));
  ...
  it("touches activity when accessing stored values", async () => {
    ...
    expect(vault.get("wallet-secret")).toEqual(new Uint8Array([1]));
  ...
  it("persists encrypted vault data to the storage adapter", async () => {
    ...
    const vault = new WalletVault(adapter, new TestCipher(), { inactivityTimeoutMs: 1_000 });
  ...
  it("restores persisted values after unlocking a new vault", async () => {
    ...
    const firstVault = new WalletVault(adapter, new TestCipher(), { inactivityTimeoutMs: 1_000 });
    ...
    const secondVault = new WalletVault(adapter, new TestCipher(), { inactivityTimeoutMs: 1_000 });

    await secondVault.unlock("test-password");

    expect(secondVault.get("wallet-secret")).toEqual(new Uint8Array([10, 20, 30]));
  });
  ...
  it("cannot persist while locked", async () => {
    ...
    await expect(vault.persist()).rejects.toThrow("Wallet vault is locked");
  });
  it("does not unlock when decryption fails", async () => {
    const adapter = new MemoryAdapter();

    await adapter.set("wallet-vault", new TextEncoder().encode("invalid-encrypted-data"));
    ...
    const vault = new WalletVault(adapter, new FailingCipher(), { inactivityTimeoutMs: 1_000 });

    await expect(vault.unlock("wrong-password")).rejects.toThrow("Decryption failed");
  ...
  it("persists and restores vault data using real Web Crypto", async () => {
    ...
    const firstVault = new WalletVault(adapter, cipher, { inactivityTimeoutMs: 1_000 });

    await firstVault.unlock("strong-test-password");

    firstVault.set("wallet-secret", new Uint8Array([10, 20, 30, 40]));
    ...
    const secondVault = new WalletVault(adapter, cipher, { inactivityTimeoutMs: 1_000 });

    await secondVault.unlock("strong-test-password");

    expect(secondVault.get("wallet-secret")).toEqual(new Uint8Array([10, 20, 30, 40]));
  });
  it("does not unlock with the wrong password", async () => {
    ...
    firstVault.set("wallet-secret", new Uint8Array([1, 2, 3]));
    ...
    await expect(secondVault.unlock("wrong-password")).rejects.toThrow("Vault decryption failed");
