...
import type { DerivedKey, WalletSignature, WalletSigner } from "./wallet-crypto-types.js";
    ...
    try {
      ...
      try {
        ...
        if (recoveredSignature.length !== 65) {
          throw new Error("Invalid recovered secp256k1 signature");
        }

        const recovery = recoveredSignature[0];

        if (recovery === undefined || recovery > 3) {
          throw new Error("Invalid secp256k1 recovery identifier");
        }

        return Object.freeze({
          compact: new Uint8Array(recoveredSignature.slice(1)),
          recovery,
        });
      } finally {
